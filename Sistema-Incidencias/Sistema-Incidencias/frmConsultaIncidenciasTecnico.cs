...
{
    public partial class frmConsultaIncidenciasTecnico : Form
    {
        private String Usuario;
        public frmConsultaIncidenciasTecnico(String Usuario)
        {
        ...
        private void frmConsultaTodasIncidenciasTecnico_Load(object sender, EventArgs e)
        {
            ...
            string Query = "SELECT ID,NUMERO_SERIE,MARCA,MODELO,AULA,FECHA,ESTATUS,DETALLE FROM VW_IncidenciasAsignadas WHERE TECNICO = " + "'" + Usuario + "'";
            ...
            if (Lector.HasRows)
            {
                ...
                while (Lector.Read())
                {
                    ...
                    string Fecha = Lector.GetValue(5).ToString();
                    string Estatus = Lector.GetValue(6).ToString();
                    string Incidencia = Lector.GetValue(7).ToString();

                    dgvTodosIncidentesTec.Rows.Add(ID, Dispositivo, Marca, Modelo, Ubicado, Fecha, Estatus, Incidencia);
                }
            }
            Conecta.Close();
        }
